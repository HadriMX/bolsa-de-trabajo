...
function getVacantes($titulo,$ubicacion,$idsueldo,$idfecha,$idarea)
{
    ...
    $stmt = $conn->prepare("SELECT * FROM vacantesvista WHERE (((titulo_vacante LIKE ?) or (nombre_empresa LIKE ?)) or ?) AND ((entidad_federativa LIKE ?) or ?) AND ((sueldo >= ? AND sueldo <= ?) or ?) AND ((fecha_publicacion >= (NOW() - INTERVAL ? DAY)) or ? ) AND ((area_estudio LIKE ?) or ? ) AND (id_vacante NOT IN (SELECT id_vacante FROM postulados WHERE id_candidato = ?)) AND estatus = 'A' ORDER BY fecha_publicacion DESC");
    $titulo = "%".$titulo."%";
    $ubicacion = "%".$ubicacion."%";
