...
export class MenuComponent implements OnInit, IAppPage {
  ...
  busquedaVacantes() {
    ...
    this.vacantesService.busquedaVacantes(this.busqueda).subscribe(response => {
      if (response.success) {
  ...
  setPage(page: number) {
    ...
    if (page < 1) {
      ...
      this.router.navigate(["/menu"], {
        queryParams: { pagina: this.pager.totalPages }
      });
      return;
    }

    // get pager object from service
    this.pager = this.PaginacionService.getPager(
    ...
    if (this.pager.currentPage >= this.pager.totalPages + 1) {
      ...
      this.router.navigate(["/menu"], {
        queryParams: { pagina: this.pager.currentPage }
      });
    } else {
      this.router.navigate(["/menu"], { queryParams: { pagina: 1 } });
    }
  ...
  getAreas() {
    this.areaService.get_areasMenu().subscribe(response => {
      if (response.success) {
  ...
  getEmailEmpresa(id_usuario){
    this.empresaService.get_emailEmpresa(id_usuario,id_usuario).subscribe(response=>{
      if(response.success){
